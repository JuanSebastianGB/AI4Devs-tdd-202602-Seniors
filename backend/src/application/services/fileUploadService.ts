...
export function uploadDiskDestination(
  _req: Request,
...
export function uploadDiskFilename(_req: Request, file: Express.Multer.File, cb: (error: Error | null, filename: string) => void) {
  const uniqueSuffix = Date.now();
  cb(null, uniqueSuffix + '-' + file.originalname);
}

export const uploadFileFilter = (req: Request, file: Express.Multer.File, cb: multer.FileFilterCallback) => {
...
const storage = multer.diskStorage({
  destination: uploadDiskDestination,
  filename: uploadDiskFilename,
});
