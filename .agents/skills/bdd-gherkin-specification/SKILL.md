...
description: Create Behavior-Driven Development (BDD) feature files using Gherkin syntax. Write clear, executable specifications that describe system behavior from the user's perspective. Use this skill whenever user asks to write feature files, create BDD scenarios, define acceptance criteria in Gherkin format, work with Cucumber/SpecFlow, or needs help with Example Mapping and Three Amigos sessions. Essential for transforming user stories into executable specifications.
version: 2.1.0
---
...
- User asks to write or create BDD scenarios/feature files
- User needs help with Gherkin syntax or keywords
- User wants to define acceptance criteria in Gherkin format
- User mentions "Three Amigos", "Example Mapping", or "Specification by Example"
- User is working with Cucumber, SpecFlow, or similar BDD tools
- User wants to convert user stories into executable specifications

## Common Use Cases

1. **Write feature files** - Create new .feature files with proper Gherkin syntax
2. **Review scenarios** - Improve existing scenarios for clarity and maintainability
3. **Convert requirements** - Transform user stories into BDD scenarios
4. **Debug Gherkin** - Fix syntax errors or anti-patterns in feature files
5. **Template generation** - Provide starting templates for new features
...
BDD works best with collaborative practices:

- **Three Amigos**: Product Owner (what), Developer (how), Tester (what could go wrong)
- **Example Mapping**: 4-color cards - User Story (blue), Rule (yellow), Example (green), Question (red)
- **Specification by Example**: Derive examples from business rules collaboratively

When helping users create BDD scenarios:
1. Ask about the business value and user goals
2. Suggest concrete examples with real data
3. Identify edge cases and error conditions
4. Ensure scenarios are independent and executable
