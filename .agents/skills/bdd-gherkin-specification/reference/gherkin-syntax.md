...
## Doc Strings for Large Content

Doc Strings allow multi-line text content in steps, useful for validating large content like JSON, HTML, or error messages.

```gherkin
Scenario: API returns validation errors
  When the user submits invalid form data
  Then the API returns the following errors:
    """
    {
      "errors": {
...
**Best practices for Doc Strings:**
- Use when validating complex data structures
- Keep indentation clean for readability
- Specify format if relevant (JSON, XML, etc.)

---

## Data Tables

Data tables provide structured input data for steps.
...
Scenario: Adding multiple products to cart
  Given the product catalog contains:
    ...
    | USB Cable     | $5.99  |
  When the user adds all products to cart
...
|------|--------|----------|
| List table | `\| col1 \| col2 \|` | Multiple rows of data |
| Hash table | `\| key = value \|` | Key-value pairs |
| Multiline | `"""text"""` | Large text content |
...
**Tag inheritance:** Tags on a Feature apply to all Scenarios within it.

---

## Comments

Use comments to add notes that won't be executed:

```gherkin
Feature: Shopping Cart

  # This feature covers the core checkout flow
  # Related issue: JIRA-1234

  Scenario: User completes purchase
    Given the cart has items
    # TODO: Add payment integration tests
    When the user proceeds to checkout
    Then the order is placed
```

**Best practice:** Use comments sparingly; prefer self-documenting scenario names and steps.

---

## Localization

Gherkin supports localized keywords. Common translations include:

| English | Spanish | French | Portuguese |
|---------|---------|--------|------------|
| Feature | Característica | Fonctionnalité | Funcionalidade |
| Scenario | Escenario | Scénario | Cenário |
| Given | Dado | Étant donné | Dado |
| When | Cuando | Quand | Quando |
| Then | Entonces | Alors | Então |

Use `# language: es` at the top of the file to use localized keywords.
