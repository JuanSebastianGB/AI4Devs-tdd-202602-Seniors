...
This skill focuses on writing Gherkin scenarios. For implementation, you'll need:

### Step Definition Mapping

Gherkin steps are mapped to code via regular expressions:
...
Extract parameters from steps using angle brackets:

```gherkin
Scenario: User views profile
  Given a user named "Alice" exists
  When "Alice" views her profile
  Then "Alice" sees their information
```

### Best Practice: Mind Map

```
User Story
    │
    ├── Business Rule 1 ── Example 1
    │                     Example 2
    │
    ├── Business Rule 2 ── Example 3
    │                     Example 4
    │
    └── Edge Cases ─────── Example 5
                          Example 6
```
